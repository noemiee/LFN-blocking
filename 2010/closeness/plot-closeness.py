...
def viz(lon0, lon1, lat0, lat1, Xi, Yi, hcl_interp, minimum, maximum, lon_r, lat_r, z_data, title, file_name) :
...
def main() :
    ...
    for d in all_dates[:-16]:
        idx+=1
        print('plotting closeness on ', d)

        # compute average closeness field over 4 days (closeness snapshots are available at 00, 06, 12, 18, ie. 4 times per day, we thus need to take the average over 16 snapshots)
        image_number = "%03d" % idx
        file = './data/closeness-fields/hcl_'+image_number+'_out.pckl'
        f = open(file, 'rb')
        hcl_data= pickle.load(f)
        f.close()
        for j in range(1,16): # add the next 15 time steps ()
            im_num = "%03d" % (idx+j)
            file = './data/closeness-fields/hcl_'+im_num+'_out.pckl'
            f = open(file, 'rb')
        ...
        interpolator = tri.LinearTriInterpolator(triang, hcl_data)
        Xi, Yi = np.meshgrid(xi, yi)
        hcl_interp = interpolator(Xi, Yi)
