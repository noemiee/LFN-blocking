...
def viz(lon0, lon1, lat0, lat1, Xi, Yi, d_interp, minimum, maximum, lon_r, lat_r, z_data, title, file_name) :
    ...
    # filled contour plot of the interpolated degree
    xxi, yyi = m(Xi, Yi)
    levels = np.linspace(minimum, maximum, 80)
    cnt = m.contourf(xxi, yyi, d_interp, levels = levels, cmap="viridis", extend = 'both')#, vmin = minimum, vmax = maximum)
...
def main() :
    ...
    for d in all_dates[:-16]:
        idx+=1
        print('plotting degree on ', d)

        # compute average degree field over 4 days (closeness snapshots are available at 00, 06, 12, 18, ie. 4 times per day, we thus need to take the average over 16 snapshots)
        image_number = "%03d" % idx
        file = './data/degree-fields/Kout'+image_number+'.pckl'
        f = open(file, 'rb')
        d_data= pickle.load(f)
        f.close()
        for j in range(1,16): # add the next 15 time steps ()
            im_num = "%03d" % (idx+j)
            file = './data/degree-fields/Kout'+im_num+'.pckl'
            f = open(file, 'rb')
            data_tmp= pickle.load(f)
            f.close()
            d_data+=data_tmp
        # average
        d_data = d_data.astype(np.float)/16.
       ...
       # interpolate 4-days-averaged closeness field for plotting (note that we are interested in latitudes 10-80ºN)
        ...
        interpolator = tri.LinearTriInterpolator(triang, d_data)
        Xi, Yi = np.meshgrid(xi, yi)
        d_interp = interpolator(Xi, Yi)
        ...
        #############################################################
        # plot degree
        viz(-175., 175., 15, 75, Xi, Yi, d_interp, minimum, maximum, lon_r, lat_r, z_data, str(all_dates[idx]), './figures/degree/Kout'+image_number+'.pdf')
